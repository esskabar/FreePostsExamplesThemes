...
if ( ! function_exists( 'hyperactivz_setup' ) ) :
    function hyperactivz_setup() {
        ...
        add_image_size( 'president-theme-main-thumbnail', 288, 310, true );
        add_image_size( 'president-theme-main-aside-thumbnail', 255, 120, true );
        add_image_size( 'president-theme-main_wide-thumbnail', 362, 200, true );
        add_image_size( 'president-theme-main-top_big-thumbnail', 581, 310, true );
        add_image_size( 'president-theme-main_small-thumbnail', 93, 68, true );
        add_image_size( 'president-theme-main_three-center-thumbnail', 170, 120, true );

        // This theme uses wp_nav_menu() in two locations.
        register_nav_menus( array(
            'primary' => __( 'Top PresidentMummy Menu', 'PresidentMummy' ),
//            'second' => __( 'Second PresidentMummy Menu', 'hyperactivz' ),
            'footer' => __( 'Footer PresidentMummy Menu', 'PresidentMummy' )
        ) );
...
function hyperactivz_scripts() {
    ...
    // Theme stylesheet.
    wp_enqueue_style( 'president-style', get_stylesheet_uri(), array('president-style-main') );
    wp_enqueue_style( 'president-style-main', TEMPLATE_DIRECTORY_URI.'/styles/main.css');




    wp_enqueue_script( 'president-vendor', TEMPLATE_DIRECTORY_URI.'/scripts/vendor.js', array(), false, true );
    wp_enqueue_script( 'president-modernizr', TEMPLATE_DIRECTORY_URI.'/scripts/vendor/modernizr.js', array(), false, false );
    wp_enqueue_script( 'president-plugins', TEMPLATE_DIRECTORY_URI.'/scripts/plugins.js', array('president-vendor'), false, true );
    wp_enqueue_script( 'president-main', TEMPLATE_DIRECTORY_URI.'/scripts/main.js', array('president-vendor', 'president-plugins' ), false, true );
...
// page option
function president_menu() {
    add_theme_page('Theme options', 'Theme options', 'edit_theme_options', 'president-theme-options', 'president_theme_options');
    add_theme_page('Banner Top', 'Banner Top', 'edit_theme_options', 'president-main-top', 'president_options_main_top');
    add_theme_page('Two right post', 'Two right post', 'edit_theme_options', 'president-options-wide-post', 'president_options_wide_post');
    add_theme_page('Center and Three Posts block with excerpt', 'Center and Three Posts block with excerpt', 'edit_theme_options', 'president-options-three-posts-block', 'president_options_three_posts_block');
    add_theme_page('Four block left', 'Four block left', 'edit_theme_options', 'president-options-three-post', 'president_options_three_post');

    add_theme_page('Four block right', 'Four block right', 'edit_theme_options', 'president-main-four-posts', 'president_main_four');
    add_theme_page('Footer posts', 'Footer posts', 'edit_theme_options', 'president-footer-posts', 'president_footer_post');
    add_theme_page('Footer posts bottom', 'Footer posts bottom', 'edit_theme_options', 'president-footer-bottom-posts', 'president_footer_bottom_post');
}
add_action('admin_menu', 'president_menu');
...
function president_theme_options() {
    ...
    <div class="wrap">
        ...
        <form action="options.php" method="POST" enctype="multipart/form-data">
            ...
            do_settings_sections( 'president_theme_options' );
            submit_button();
            ?>
        </form>
    </div>
...
function theme_options_settings(){
    ...
    add_settings_section( 'theme_options_id', 'Customize president Theme', '', 'president_theme_options' );


    add_settings_field('custom_logo', 'Logo image:<br>max size 300x47', 'fill_logo_image', 'president_theme_options', 'theme_options_id' );
}
...
//main top Banner
function president_options_main_top() {
    if (!current_user_can('manage_options')) {
        wp_die(__('You do not have sufficient permissions to access this page.'));
    }
    ?>
...
function plugin_settings_main_top(){
    ...
    add_settings_field('main_top_post_right_first', 'Show post on main top<br>Left Block (left position): (paste ID)', 'fill_main_top_right', 'main_top_block', 'main_top_section_id' );

    add_settings_field('main_top_post_right_second', 'Show post on main top<br>Left Block (right position): (paste ID)', 'fill_main_top_right_second', 'main_top_block', 'main_top_section_id' );
...
function fill_main_top_right(){
    $val = get_option('main_top_post_id');
    $val = $val['left_first'];
    ?>
    <input type="text" name="main_top_post_id[left_first]" value="<?php echo esc_attr( $val ) ?>" />
<?php
}
function fill_main_top_right_second(){
    $val = get_option('main_top_post_id');
    $val = $val['left_second'];
    ?>
    <input type="text" name="main_top_post_id[left_second]" value="<?php echo esc_attr( $val ) ?>" />
<?php
}
...
//first three main posts block
function president_options_three_posts_block() {
    if (!current_user_can('manage_options')) {
        wp_die(__('You do not have sufficient permissions to access this page.'));
    }
    ?>
    <div class="wrap">
        ...
        <form action="options.php" method="POST">
            ...
            do_settings_sections( 'president-options-three-posts-block' );
            submit_button();
            ?>
        </form>
    </div>
...
function plugin_settings_three_posts_block(){
    ...
    register_setting( 'three_posts_block', 'three_posts_block_center', 'sanitize_callback' );

    add_settings_section( 'three_posts_block_section', 'Customize Block with three posts section with excerpt', '', 'president-options-three-posts-block' );

    add_settings_field('three_posts_block', 'Show three posts with excerpt:<br>(paste post ID, comma separated)', 'fill_three_post_input', 'president-options-three-posts-block', 'three_posts_block_section' );

    add_settings_field('three_posts_block_big', 'Show three next posts with excerpt:<br>(paste post ID, comma separated)', 'fill_big_post_input', 'president-options-three-posts-block', 'three_posts_block_section' );


}
function fill_three_post_input(){
    $val = get_option('three_posts_block_center');
    $val = $val['three_posts'];
    ?>
    <input type="text" name="three_posts_block_center[three_posts]" value="<?php echo esc_attr( $val ) ?>" />
<?php
}
function fill_big_post_input(){
    $val = get_option('three_posts_block_center');
    $val = $val['second_center_excerpt'];
    ?>
    <input type="text" name="three_posts_block_center[second_center_excerpt]" value="<?php echo esc_attr( $val ) ?>" />
<?php
}


//Four block left (Big and Small)
function president_options_three_post() {
    if (!current_user_can('manage_options')) {
        wp_die(__('You do not have sufficient permissions to access this page.'));
    }
    ?>
    <div class="wrap">
        ...
        <form action="options.php" method="POST">
            <?php
            settings_fields( 'four_post' );
            do_settings_sections( 'president-options-three-post' );
            submit_button();
            ?>
        </form>
    </div>
...
function plugin_settings_three_post(){
    ...
    register_setting( 'four_post', 'four_post_id', 'sanitize_callback' );


    add_settings_section( 'four_post_section', 'Customize Block with Two section', '', 'president-options-three-post' );


    add_settings_field('four_post_left_big', 'Show Big post in left section:<br>(paste post ID', 'fill_four_post_input_left', 'president-options-three-post', 'four_post_section' );

    add_settings_field('four_post_left_small', 'Show three Small posts in left section:<br>(paste post ID', 'fill_three_small_post_input_left', 'president-options-three-post', 'four_post_section' );

}
function fill_four_post_input_left(){
    $val = get_option('four_post_id');
    $val = $val['left_big'];
    ?>
    <input type="text" name="four_post_id[left_big]" value="<?php echo esc_attr( $val ) ?>" />
<?php
}
function fill_three_small_post_input_left(){
    $val = get_option('four_post_id');
    $val = $val['left_small'];
    ?>
    <input type="text" name="four_post_id[left_small]" value="<?php echo esc_attr( $val ) ?>" />
<?php
}



//block with two wide post right
function president_options_wide_post() {
    if (!current_user_can('manage_options')) {
        wp_die(__('You do not have sufficient permissions to access this page.'));
    }
    ?>
    <div class="wrap">
        ...
        <form action="options.php" method="POST">
            ...
            do_settings_sections( 'president-options-wide-post' );
            submit_button();
            ?>
        </form>
    </div>
...
function plugin_settings_wide_post(){
    ...
    add_settings_section( 'block_wide_post_section', 'Customize block with wide post', '', 'president-options-wide-post' );

    add_settings_field('two_post_wide_right', 'Show post in wide section<br>ONE WIDE: (paste ID)', 'fill_two_wide_post_id', 'president-options-wide-post', 'block_wide_post_section' );

}
function fill_two_wide_post_id(){
    $val = get_option('block_wide_post');
    $val = $val['right'];
    ?>
    <input type="text" name="block_wide_post[right]" value="<?php echo esc_attr( $val ) ?>" />
<?php
}


////Four block right (Big and Small)
function president_main_four() {
    if (!current_user_can('manage_options')) {
        wp_die(__('You do not have sufficient permissions to access this page.'));
    }
    ?>
    <div class="wrap">
        ...
        <form action="options.php" method="POST">
            <?php
            settings_fields( 'block_with_main_four_posts' );
            do_settings_sections( 'president-main-four-posts' );
            submit_button();
            ?>
        </form>
    </div>
...
add_action('admin_init', 'plugin_settings_main_six_post');
function plugin_settings_main_six_post(){

    register_setting( 'block_with_main_four_posts', 'block_main_four_posts', 'sanitize_callback' );

    add_settings_section( 'block_main_four_section', 'Customize block right four posts', '', 'president-main-four-posts' );

    add_settings_field('post_wide_right_big', 'Show Big post in right section<br>ONE WIDE: (paste ID)', 'fill_main_four_post_id_big', 'president-main-four-posts', 'block_main_four_section' );

    add_settings_field('three_post_wide_right', 'Show threee Small posts in right section<br>ONE WIDE: (paste ID)', 'fill_main_four_post_id_small', 'president-main-four-posts', 'block_main_four_section' );

}
function fill_main_four_post_id_big(){
    $val = get_option('block_main_four_posts');
    $val = $val['right_big'];
    ?>
    <input type="text" name="block_main_four_posts[right_big]" value="<?php echo esc_attr( $val ) ?>" />
<?php
}


function fill_main_four_post_id_small(){
    $val = get_option('block_main_four_posts');
    $val = $val['right_small'];
    ?>
    <input type="text" name="block_main_four_posts[right_small]" value="<?php echo esc_attr( $val ) ?>" />
<?php
}
...
////Footer posts
function president_footer_post() {
    if (!current_user_can('manage_options')) {
        wp_die(__('You do not have sufficient permissions to access this page.'));
    }
    ?>
    <div class="wrap">
        ...
        <form action="options.php" method="POST">
            <?php
            settings_fields( 'block_footer_posts' );
            do_settings_sections( 'president-footer-posts' );
            submit_button();
            ?>
        </form>
    </div>
...
add_action('admin_init', 'plugin_settings_footer_post');
function plugin_settings_footer_post(){

    register_setting( 'block_footer_posts', 'block_footer_posts_right', 'sanitize_callback' );

    add_settings_section( 'block_footer_section', 'Customize block footer posts', '', 'president-footer-posts' );

    add_settings_field('footer_posts', 'Show post in wide section<br>ONE WIDE: (paste ID)', 'fill_footer_post_id', 'president-footer-posts', 'block_footer_section' );

}
function fill_footer_post_id(){
    $val = get_option('block_footer_posts_right');
    $val = $val['top'];
    ?>
    <input type="text" name="block_footer_posts_right[top]" value="<?php echo esc_attr( $val ) ?>" />
<?php
}
...
//Footer bottom posts
function president_footer_bottom_post(){
    if (!current_user_can('manage_options')) {
        wp_die(__('You do not have sufficient permissions to access this page.'));
    }
    ?>
    <div class="wrap">
        ...
        <form action="options.php" method="POST">
            <?php
            settings_fields( 'block_footer_bottom_posts' );
            do_settings_sections( 'president-footer-bottom-posts' );
            submit_button();
            ?>
        </form>
    </div>
...
add_action('admin_init', 'plugin_settings_footer_bottom_post');
function plugin_settings_footer_bottom_post(){

    register_setting( 'block_footer_bottom_posts', 'block_footer_posts_bottom', 'sanitize_callback' );

    add_settings_section( 'block_footer_bottom_section', 'Customize block footer bottom posts', '', 'president-footer-bottom-posts' );

    add_settings_field('footer_posts_bottom', 'Show post in wide section<br>ONE WIDE: (paste ID)', 'fill_footer_bottom_post_id', 'president-footer-bottom-posts', 'block_footer_bottom_section' );

}
function fill_footer_bottom_post_id(){
    $val = get_option('block_footer_posts_bottom');
    $val = $val['down'];
    ?>
    <input type="text" name="block_footer_posts_bottom[down]" value="<?php echo esc_attr( $val ) ?>" />
<?php
}
...
function register_my_widgets(){
    ...
    register_sidebar( array(
        ...
        'before_title' => '<h2 class="title_block">',
        'after_title' => '</h2><div class="wrap_widget">'
    ) );
    register_sidebar( array(
        'name'          => 'Below Post',
